...
- **포인터 연산**
  ...
  포인터 연산은 포인터 변수에 +, - 연산자를 사용하여 값을 더하거나 빼서 다음 또는 이전 메모리에 접근한다. 단 &#42;,/ 연산자와 실수 값은 사용할 수 없다.
  ...
  결과를 보면 1을 더했는데 메모리 주소는 4만큼 차이가 난다. 즉 포인터 연산은 **포인터 자료형의 크기**만큼 더하거나 뺀다. 계산 식으로 나타내면 **"sizeof(자료형) x 더하거나 뺴는 값"**이 된다.

  ![포인터와 메모리 주소](https://dojang.io/pluginfile.php/573/mod_page/content/24/unit59-2.png)

  뺄셈도 마찬가지다.

  - 증감 연산자 역참조 시 주의점

    증감 연산자와 역참조를 동시에 사용할 때는 꼭 증감 연산자를 변수 앞에 붙여주어야 한다.

    그 이유는 증감 연산자를 변수 뒤에 붙이게 되면 현재 메모리의 값을 가져온 뒤 포인터 연산을 하기 때문이다.
  ...
  - void 포인터는 자료형의 크기가 정해져 있지 않기 때문에 포인터 연산이 불가능하다. 포인터 연산을 하려면 형 변환이 필요하다.

  - 동적 메모리를 할당받은 포인터를 ++, -- 연산자로 포인터 연산을 하게 되면 포인터에 저장된 메모리 주소 자체가 바뀌게 된다. 이 때 free함수에서 메모리 주소가 바뀐 포인터로 메모리 해제를 하면 에러가 발생하므로 주의한다. 
    ...
    *(++(int *)ptr) = 10;    // 증가 연산자를 사용했으므로 4만큼 증가한 메모리 주소가 ptr에 다시 저장됨
    ...
    free 함수로 메모리 해제를 할 때는 반드시 처음에 메모리를 할당할 때 받은 주소(포인터)를 넣어주어야 한다.

  - C 언어 표준에서 허용하지 않지만 Visual Studio에서는 사용 할 수 있는 것들

    - 변수의 자료형을 다른 자료형으로 변환하며 동시에 변수의 값을 변경하는 것
    ...
    - 포인터를 다른 자료형 포인터로 변환하며 동시에 ++, -- 연산자를 사용하는 것
      ...
      이 코드를 GCC에서 컴파일하려면 포인터를 다른 자료형의 포인터에 할당한 뒤 ++, -- 연산자를 사용해야 한다.
      ...
      C 언어 표준에서 자료형 변환은 l-value를 생성하지 않는다고 규정되어 있다. 참고로 l-value는 메모리 공간을 차지하는 표현식을 뜻하고 r-value는 l-value 이외의 표현식을 뜻한다.

---

- **Vs 디버깅에서 F5키의 역할**
  - 디버깅을 시작하는 기능
  - 디버깅 중일 때는 다음 중단점까지 프로그램을 계속 실행하는 기능을 한다.
  - 따라서 함수의 시작점 마다 중단점을 설정하고 디버깅을 하면 함수의 실행 순서를 파악할 수 있다.
  - ![함수 디버깅](https://dojang.io/pluginfile.php/1023/mod_page/content/4/unit60-2.png)
...
- **매개변수, parameter, 인자, 인수, argument**
  ...
  - 매개변수, parameter, 형식 매개변수(formal parameter), 인자 :
  ...
  - 인수, 전달인자, argument, 실행 전달인자(actual argument) :
...
- **Debug 단축키**
  - F11(한 단계씩 코드 실행) : 함수 호출 부분에 중단점을 설정하고 F11을 누르면 함수 안으로 이동한다. (현재 코드가 함수 호출이 아닐 때는 F10처럼 다음 줄로 이동한다.)
  - Shift + F11(프로시저 나가기) : 함수를 빠져나가 다시 함수 호출 부분으로 이동한다. 함수를 빠져나오면 반환값이 표시된다.
...
- **배열을 매개변수로 사용하기**
  ...
  - 배열과 그 크기를 인자로 넣어 줄 때는 크기를 sizeof(arr) / sizeof(자료형) 으로 계산해서 넣어준다. 이렇게 하면 배열의 크기가 바뀌어도 코드를 수정하지 않아도 된다.
    ...
    ![매개변수로 받은 배열의 요소를 변경](https://dojang.io/pluginfile.php/626/mod_page/content/34/unit64-1.png)

    setElement 함수 안에서 arr[2]에 300을 저장했다. 그다음에 main 함수에서 numArr[2]를 출력해보면 300이 나온다. 즉, 배열의 메모리 주소를 전달했기 때문에 실제로는 바깥에 있는 배열의 요소가 바뀌었다.

  - 함수에 배열을 넘겨줄 때 복합 리터럴을 사용하면 배열을 따로 선언하지 않아도 된다. (C99에 추가된 기능)
  ...
  - 함수의 배열 매개변수에 static과 숫자를 사용하면 요소의 최소 개수를 지정할 수 있다. 이렇게 하면 매개변수로 들어온 배열의 요소 개수가 지정된 개수보다 작을 때 경고가 발생한다. 단 GCC, Clang에서만 지원한다.
  ...
  - 함수에서 2차원 배열을 매개변수로 사용하려면 다음과 같이 지정한다.
  ...
  - 2차원 배열을 함수에 넘겨줄 때 복합 리터럴을 사용하는 법은 다음과 같다.
...
- **구조체 매개변수로 사용하기**
  ...
  - 구조체를 직접 사용하는 것 보다 구조체 포인터를 사용해서 인자로 받는 것이 훨씬 공간적인 측면에서 효율적이다. 구조체 뿐만 아니라 열거형, 공동체도 모두 매개변수로 받을 수 있다.
...
- **함수 포인터**
  ...
  함수를 배열 또는 구조체에 넣거나, 함수 자체를 함수의 매개변수로 넘겨주고, 반환값으로 가져올 때는 함수 포인터를 사용한다.
...
- **퀵 정렬 함수**
  ...
  - 다음 코드를 보자
    ...
    먼저 qsort 함수를 사용하기 전에 비교(compare) 함수를 만들어야 합니다.  비교 함수를 정의할 때는 반드시 int형 반환값과 const void 포인터 매개변수가 두 개 있어야 합니다. 하지만 const void 포인터로는 값을 비교할 수 없으므로 정렬할 배열의 자료형에 따라 const void 포인터를 변환한 뒤 역참조하여 값을 가져옵니다. 여기서는 정렬할 배열이 int형이므로 const void 포인터를 int 포인터로 변환한 뒤 역참조하여 값을 가져왔습니다.
